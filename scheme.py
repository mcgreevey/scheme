...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 10
        env.define(target.first, do_lambda_form(Pair(target.second, Pair(expressions.second.first, expressions.second.second)), env))
        return target.first
...
def do_and_form(expressions, env):
    ...
    while expressions != nil:
        if expressions.second != nil:
            Circuit = scheme_eval(expressions.first, env)
            if scheme_falsep(Circuit): #short circuit
                return False
        else:
            Circuit = scheme_eval(expressions.first, env, True)
            if scheme_falsep(Circuit): #short circuit
                return False
